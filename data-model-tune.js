/* ============================================================
   ...
   ── board3d.js（ゲーム本体）と sim-editor.html（エディタ）の両方がこのファイルを読む。
      以前は同じ内容を2ファイルに手で書き写す運用だったが、片方だけ更新して
      食い違う事故が起きやすかったため、ここ1箇所にまとめた。
   ── エディタで調整 →「⬇ data-model-tune.js をダウンロード」→ このファイルを差し替え。
      board3d.js を触る必要はない。

   MODEL_TUNE       … 大きさ・位置・向きの微調整
        scale … 1.0 が標準の背丈。0.9 で少し小さく。
        x / z … 立ち位置のズレ直し（マス1個の半径 = 1.0 が基準）
        y     … 高さ。足が沈む／浮くときだけ触る。
        rotY  … 向き（ラジアン。エディタが自動で変換して書き出す）

   MODEL_HIDE_PARTS … 消したいパーツ（名前の一部でも可・大文字小文字は無視）
   MODEL_KEEP_PARTS … 自動判定で消えてしまうが残したいパーツ
        例) ヴァルスの弓は 'Weapon1' という名前で、本体から離れて持たれているため
            「浮いた飛翔体」と誤判定されて消える。そこで keep に入れて保護している。
   ============================================================ */
