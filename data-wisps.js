...
const WISPS_DATA = [
  ...
  { id:"wisp_ominous_deal", name:"不吉な取引", category:"misc", cost:0, icon:"🩸", desc:"プレイヤー体力を4失う。2ゴールドを獲得する。", effect:(state, rng, helpers) => { helpers.addGold(2); helpers.showMsg('🩸 不吉な取引: 2G獲得！'); }, tier:2 },
  ...
  { id:"wisp_gadget_bottle", name:"装置瓶", category:"misc", cost:0, icon:"⚙️", desc:"物理防御と魔法防御、または体力を付与するランダムな消費アイテムを1個獲得する。", effect:(state, rng, helpers) => { const it = ITEMS[Math.floor(rng() * ITEMS.length)]; helpers.addItem({ ...it }); helpers.showMsg(`⚙️ 装置瓶: ${it.jaName}を獲得！`); }, tier:2 },
  { id:"wisp_overflow_water", name:"あふれる水", category:"risky", cost:1, icon:"💦", desc:"連勝数が1増える。", effect:(state, rng, helpers) => { helpers.addPassiveBuff({ type: 'wisp_winstreak_plus' }); helpers.showMsg('💦 あふれる水: 連勝数+1（記録のみ）'); } },
  ...
  { id:"wisp_have_one", name:"おひとつどうぞ", category:"risky", cost:1, icon:"🎁", desc:"敗北しても敵を1体以上キルしていれば、4XPを獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🎁 おひとつどうぞ:（対戦結果に依存するため今回は効果なし）'); } },
  { id:"wisp_minor_chaos", name:"下級カオス", category:"misc", cost:1, icon:"🌀", desc:"ランダムなウィスプを1つ使用する。", effect:(state, rng, helpers) => {
  ...
  { id:"wisp_opening_book", name:"初級の定跡", category:"risky", cost:1, icon:"📖", desc:"次の対人戦で勝利すると、2ゴールドを獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('📖 定跡(初級):（対戦結果に依存するため今回は効果なし）'); } },
  { id:"wisp_drought", name:"日照り", category:"risky", cost:1, icon:"☀️", desc:"連敗数が2増えます。", effect:(state, rng, helpers) => { helpers.addPassiveBuff({ type: 'wisp_lossstreak_plus' }); helpers.showMsg('☀️ 日照り: 連敗数+2（記録のみ）'); } },
  { id:"wisp_mitosis", name:"有糸分裂", category:"champion", cost:1, icon:"🧫", desc:"「小さなチャンピオン複製器」を1個獲得する。", effect:(state, rng, helpers) => { helpers.addItem({ ...CONSUMABLES.TINY_DUPE }); helpers.showMsg('🧫 有糸分裂: 小さなチャンピオン複製器を獲得！'); } },
  { id:"wisp_life_drain", name:"生気吸引", category:"risky", cost:1, icon:"🩸", desc:"この戦闘で追加の「魔女」エッセンスを50%獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🩸 生気吸引:（このシムは戦闘を扱わないため効果なし）'); } },
  ...
  { id:"wisp_fertilizer", name:"肥料", category:"risky", cost:1, icon:"🌾", desc:"敵がキルされるたびに、経験値を1XP獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🌾 肥料:（このシムは戦闘を扱わないため効果なし）'); } },
  ...
  { id:"wisp_trophy_hunter", name:"トロフィー ハンター", category:"risky", cost:2, icon:"🏆", desc:"ライバルからのキル/アシストを追加で100%獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🏆 トロフィー ハンター:（このシムは戦闘を扱わないため効果なし）'); } },
  ...
  { id:"wisp_propagation", name:"伝播", category:"champion", cost:2, icon:"🧬", desc:"最初に倒された味方チャンピオンのコピーを1体獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🧬 伝播:（このシムは戦闘を扱わないため効果なし）'); }, tier:2 },
  { id:"wisp_healing_pool", name:"回復プール", category:"misc", cost:2, icon:"💧", desc:"プレイヤー体力を5回復する。", effect:(state, rng, helpers) => { helpers.showMsg('💧 回復プール:（体力システムがこのシムに無いため効果なし）'); }, tier:2 },
  { id:"wisp_golden_path", name:"黄金の道", category:"risky", cost:2, icon:"🛤️", desc:"対人戦を3回行ったあと、1勝利あたり2ゴールドを獲得する。", effect:(state, rng, helpers) => { helpers.showMsg('🛤️ 黄金の道:（対戦結果に依存するため今回は効果なし）'); }, tier:2 },
  ...
  { id:"wisp_bronze_spoon", name:"ブロンズ スプーン", category:"economy", cost:3, icon:"🥄", desc:"経験値を4 XP獲得する。", effect:(state, rng, helpers) => { helpers.addXp(4); helpers.showMsg('🥄 ブロンズ スプーン: 4XP獲得！'); }, tier:2 },
  { id:"wisp_blood_ritual", name:"血の儀式(初級)", category:"misc", cost:3, icon:"🩸", desc:"プレイヤー体力を3失う。「小型チャンピオン複製器」を1個獲得する。", effect:(state, rng, helpers) => { helpers.addItem({ ...CONSUMABLES.LESSER_DUPE }); helpers.showMsg('🩸 血の儀式(初級): 小型チャンピオン複製器を獲得！'); }, tier:2 },
